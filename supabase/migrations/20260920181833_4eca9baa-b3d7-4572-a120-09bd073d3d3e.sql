DROP INDEX IF EXISTS public.ux_nwg_images_unique;

CREATE OR REPLACE FUNCTION public.invoke_sync_function(fn text, qs text DEFAULT ''::text)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO ''
...
DECLARE key text;
BEGIN
  -- Every scheduled/manual launch first closes abandoned runs. This avoids a
  -- permanent polling job while keeping the status table truthful.
  PERFORM public.reap_stale_syncs(30);
...
BEGIN
  ...
  -- NWG public net price: Ervitex contract price multiplied by 1.65. The
  -- storefront adds 21% VAT once when displaying the inclusive price.
  INSERT INTO public.catalog_variant_prices (source, sku, style_code, color_code, size, retail_price, currency)
  SELECT 'nwg', k.sku, k.product_number, v.color_code, k.size,
         ROUND((k.purchase_price * 1.65)::numeric, 2), COALESCE(k.purchase_currency, 'EUR')
