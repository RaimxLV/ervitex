...
drop function if exists public.price_audit_expected();
...
as $$
  ...
  where cv.source = 'nwg'
  union all
  select cv.source, cv.sku, cv.style_code, l.price, round(l.price, 2), cv.retail_price
  from catalog_variant_prices cv
  left join lateral (
...
revoke all on function public.price_audit_summary() from public, anon;
revoke all on function public.price_audit_mismatches(text, int) from public, anon;
